...
const NavBar = () => {
  ...
  const {setShowSearch, getCartItems, navigate, token, setToken, backend_url, setCartItems} = useContext(ShopContext);
    ...
    <div className="flex items-center justify-between py-5 font-medium">
      ...
      <div className="flex items-center gap-5 relative">
        <div className="group relative">
          <User onClick={() => token ? navigate('/profile') : navigate('/login')} className="w-5 h-5 cursor-pointer" />
          {token &&
            <div className="absolute right-0 pt-4 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-300">
              <div className="flex flex-col gap-2 w-36 py-2 px-4 bg-slate-100 border border-gray-500 text-slate-500 rounded shadow-lg">
                <p onClick={handleLogout} className="cursor-pointer hover:text-black text-sm">Logout</p>
                <Link to="/profile" className="cursor-pointer hover:text-black text-sm">Profile</Link>
      ...
      <div className={`fixed top-0 right-0 bottom-0 overflow-hidden bg-white transition-all duration-500 ${open ? "w-full" : "w-0"}`}>
        <div className="flex flex-col text-gray-600">
          <div onClick={() => setOpen(false)} className="flex items-center gap-5 p-3 cursor-pointer">
            <ArrowLeft className="w-5 h-5 cursor-pointer" />
            <p className="text-lg text-sm font-bold">Menu</p>
          </div>
          {["Home", "Collection", "About", "Contact"].map((item, index) => (
          ...
          {token && (
            <>
              <NavLink onClick={() => setOpen(false)} to="/orders" className="py-2 pl-6 border border-b border-gray-200">Orders</NavLink>
              <p onClick={handleLogout} className="py-2 pl-6 border border-b border-gray-200 cursor-pointer">Logout</p>
            </>
          )}
        </div>
      </div>
